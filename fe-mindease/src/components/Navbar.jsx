        ...
        <div className="hidden md:flex items-center gap-2 shrink-0">
          ...
          {!token ? (
            ...
            <div className="flex items-center gap-1">
              <Link to="/settings" className="flex items-center gap-2 hover:bg-gray-100 dark:hover:bg-gray-800 p-1 pr-3 rounded-full transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-700" title="Pengaturan Akun">
                {user?.profile_picture ? (
                  <img src={user.profile_picture} alt="Profile" className="w-8 h-8 rounded-full object-cover border border-gray-300 dark:border-gray-600" />
                ) : (
                  <div className="w-8 h-8 rounded-full bg-emerald-500 text-white flex items-center justify-center font-bold text-sm">
                    {user?.username?.charAt(0).toUpperCase()}
                  </div>
                )}
                <span className="text-sm font-medium max-w-[80px] truncate" style={{ color: 'var(--t-primary)' }}>{user?.username}</span>
              </Link>
              <button onClick={logout}
                className="flex items-center justify-center w-10 h-10 text-rose-400 hover:text-rose-300 rounded-full transition-colors"
          ...
          {!token ? (
            ...
            <>
              <Link to="/settings" onClick={() => setIsMobileMenuOpen(false)}
                className="w-full flex items-center gap-3 py-3 px-4 text-sm font-medium rounded-xl transition-colors"
                style={{ background: 'var(--bg-subtle)', color: 'var(--t-primary)' }}>
                {user?.profile_picture ? (
                  <img src={user.profile_picture} alt="Profile" className="w-6 h-6 rounded-full object-cover" />
                ) : (
